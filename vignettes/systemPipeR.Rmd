...
from an intial metadata table (targets file), R code and CWL parameter files for each 
R- and CL-based analysis step in analysis workflows (details provided below). For preconfigured workflows, users need to provide only a targets file and the initial input data specified in that file (such as FASTQ files). The
targets file provides information about the input data and additional metadata
describing the experimental design, such as sample labels, replicate
...
_`systemPipeR`_ adopts the Common Workflow Language (CWL) [@Amstutz2016-ka], which is a
...
cwl and yml files directly. The structure of `targets` files is explained in the corresponding
section [below](#targets-files). A detailed overview of the CWL syntax is provided in the 
[CWL syntax](#cwl) section below, and the details for connecting the input information in `targets` 
with CWL parameters are described [here](#cwl_targets). 
...
and `samtools` are listed. If the `in_path` columns shows `FALSE` for 
...
specified in the header lines, starting with the tag '``# <CMP>``', of a `targets` file. 
...
is important for tracking the run status of workflows and supporting restart functionality for 
workflows. 
...
The function checks whether the expected workflow directories [here](#wf-directories) are present,
...
At this stage, `sal` is an empty `SYSargsList` (`SAL`) container that only contains
...
Workflows in `systemPipeR` can be constructed stepwise in [interactive mode](#appendstep), or by importing all steps with a [single command](#importWF) from an R Markdown workflow file. This tutorial section introduces both options. In both cases, workflows are constructed by connecting data analysis steps with the `appendStep` method that can contain R code or CL calls.

## Stepwise construction  {#appendstep}

The following demonstrates how to design, load and run workflows using a simple data processing routine as an example. This mini workflow will export a test dataset to multiple files, compress/decompress them, import them back into R, and then perform a simple statistical analysis. 

The `sal` object of the new workflow project (directory named`.SPRproject`) was intialized in the previous section.
At this point this `sal` instance contains no data analysis steps since none have been loaded so far.
...
The first workflow step comprises R code that needs to be declared with the `LineWise` function, and loaded into `sal` with`appendStep`. The R code of an analysis step needs to be assigned to the `code` argument. In this assignment the R code has to be enclosed by braces (`{...}`) and separted from them by new lines. Additionally, the workflow step needs to be named under the `step_name` argument. Step names are required to be unique throughout workflows. During the construction of workflow steps, the included R code will not be executed. The execution of workflow steps is explained in as separate section [here](#wf-execution).

In the code example used here, the `iris` dataset will be split by the species names under the `Species` column, and then the resulting `data.frames` are exported to tabular files. 
...
To extract the code of an R step, the `codeLine` method can be used
...
# Running the workflow {#wf-execution}
